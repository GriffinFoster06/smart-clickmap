// Smart Clickmap - Standalone OBS Overlay
// Adapted from original overlay.js with local server support

(function () {
    'use strict';

    const EBS = window.location.origin;  // Changed from hardcoded URL to local origin
    ...
    class OptimalHybridOverlay {
        constructor() {
            ...
            console.log('🎯 Standalone overlay initializing...');
            this.init();
        }
        ...
        async init() {
            ...
            console.log('🎯 Standalone overlay ready');
        }
        ...
        async initializeWebSocket() {
            ...
            try {
                const wsUrl = `${EBS.replace('https://', 'wss://').replace('http://', 'ws://')}/ws`;
        ...
        scheduleWebSocketReconnect() {
            if (this.wsReconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
                console.log('⚠️ Max WebSocket reconnect attempts reached');
                return;
            }
        ...
        async httpPoll() {
            ...
            try {
                const data = await this.cachedFetch(`${EBS}/heatmap`);
        ...
        handleMessage(data, source) {
            ...
            if (action === 'reset' || allDataCleared || hardReset) {
                console.log(`🗑️ RESET via ${source}`);
        ...
        updateVisualization(data, source) {
            ...
            if (source.includes('reset') || allDataCleared || hardReset || action === 'reset') {
    ...
    function initialize() {
        try {
            ...
            console.log('🎯 Standalone overlay loaded');
